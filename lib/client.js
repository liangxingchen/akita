    ...
    r.then = function (onSuccess, onFail) {
      if (!resultPromise) {
        resultPromise = r.response().then(function (response) {
          if (debug) debug('response status:', response.status);
          if (response.status === 404 && (init.method === 'GET' || init.method === 'DELETE')) {
            if (debug) debug('return null when ' + init.method + ' 404');
            return null;
          }
          return response.json().then(function (data) {
            if (debug) debug('response json:', data);
